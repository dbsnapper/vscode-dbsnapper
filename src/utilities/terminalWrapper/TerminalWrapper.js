...
import {
  ...
  pathNormalizedForce,
  whichNormalized,
} from "@dbux/common-node/src/util/pathUtil";
// import sleep from '@dbux/common/src/util/sleep';
import {
  closeDefaultTerminal,
  runInTerminal,
  runInTerminalInteractive,
} from "../codeUtil/terminalUtil";
import {
  getShellPath,
  getNodePath,
  getResourcePath,
} from "../codeUtil/codePath";
...
export default class TerminalWrapper {
  ...
  async _run(cwd, command, options, isInteractive = false) {
    ...
    let tmpFolder = pathNormalized(
      fs.mkdtempSync(path.join(os.tmpdir(), "dbux-"))
    );
    tmpFolder = pathNormalizedForce(tmpFolder);

    const pathToNode = getNodePath();
    const shell = getShellPath();
    const pathToDbuxRun = pathNormalizedForce(
      getResourcePath("src/_dbux_run.js")
    );
    ...
    const serializedRunJsArgs = Buffer.from(JSON.stringify(runJsargs)).toString(
      "base64"
    );
    // const runJsCommand = `pwd && node -v && which node && echo %PATH% && node ${pathToDbuxRun} ${serializedRunJsArgs}`;
    const runJsCommand = `"${pathToNode}" "${pathToDbuxRun}" ${
      !!isInteractive + 0
    } ${serializedRunJsArgs}`;

    debug(
      "wrapping terminal command: ",
      JSON.stringify(runJsargs),
      `pathToDbuxRun: ${pathToDbuxRun}`
    );
    ...
    watcher.on("change", (eventType, filename) => {
      ...
      let result;
      if (filename === "error") {
        let errorString = fs.readFileSync(path.join(tmpFolder, filename), {
          encoding: "utf8",
        });
        let error = JSON.parse(errorString);
        _reject(
          new Error(
            `Terminal wrapper received error: ${
        ...
        Verbose &&
          debug("Terminal command finished. Result:", JSON.stringify(result));
        _resolve(result);
        resolved = true;
      }
    ...
    watcher.on("error", (err) => {
      let newErr = new NestedError(
        `[FSWatcher error while waiting for TerminalWrapper]`,
        err
      );
      if (resolved) {
        warn(newErr);
